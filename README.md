<!DOCTYPE html>
<html lang="en">
    <body>
        <div>
            <section>
                <div>
                    <h1>
                        Adrian Milano, S.Pd
                    </h1>
                    <div>
                        Kramat Jati, Jakarta Timur · (+62) 8112-175-175 ·
                        <a href="mailto:arm.adrian02@gmail.com">arm.adrian02@gmail.com</a>
                    </div>
                   <p>I am a highly passionate IT and programming enthusiast with expertise in both frontend and backend development. I am an energetic programmer who is dedicated to delivering high-quality results in all my projects.</p>
                    <p>Based on my previous experience, I have worked as both a web frontend and backend developer. I am passionate about my work and am comfortable working under pressure. Additionally, I enjoy collaborating with a team to achieve a common goal.</p>
                    <p>I have a strong curiosity and a deep passion for learning new things, especially in the field of IT and programming languages.</p>
                </div>
            </section>
            <hr />
            <section>
                <div>
                    <h2>Experience</h2>
                    <div>
                        <div>
                            <h3>Senior Frontend Enginner</h3>
                            <div><a href="https://www.paper.id/" target="_blank">PT. Pakar Digital Global (Paper.id)</a></div>
                            <p>As a senior frontend web developer specializing in the use of the Angular programming language, I assist in the development of micro-based applications for clients, especially Enterprise Products, using clean architecture methods, trunk-based development and test-based development practices. Apart from that, I use Scrum for project management. Products that focus on finance and invoicing.</p>
                        </div>
                        <div>June 2023 - Present</div>
                    </div>
                    <div>
                        <div>
                            <h3>Senior Frontend Developer</h3>
                            <div><a href="https://salt.id/" target="_blank">PT. Ako Media Asia (SALT)</a></div>
                            <p>As a senior frontend web developer who specializes in using the Angular programming language, I assist in the development of micro-based applications for clients, particularly Telkomsel, utilizing clean architecture methods, trunk-based development, and test-driven development practices. Additionally, I employ Scrum for project management.</p>
                        </div>
                        <div>February 2022 - June 2023</div>
                    </div>
                    <div>
                        <div>
                            <h3>Tech Manager</h3>
                            <div><a href="https://mogawe.id/" target="_blank">PT. Media Baru Internasional (MoGawe)</a></div>
                            <p>As a technology manager, my responsibilities include conducting research to implement the latest technological advancements and leading a team of frontend web developers skilled in Angular, backend developers using Java Spring Boot, and mobile application developers proficient in Flutter. I also oversee server management using the Google Cloud Console and maintain direct coordination with the CEO to ensure efficient company management.</p>
                        </div>
                        <div>December 2021 - May 2023</div>
                    </div>
                    <div>
                        <div>
                            <h3>Web Developer</h3>
                            <div><a href="https://mogawe.id/" target="_blank">PT. Media Baru Internasional (MoGawe)</a></div>
                            <p>As a web frontend developer skilled in Angular, I specialize in developing micro-based applications, including work management and data research applications. I also implement Scrum project management methodologies to ensure the success of my projects.</p>
                        </div>
                        <div>August 2020 - December 2021</div>
                    </div>
                    <div>
                        <div>
                            <h3>Fullstack Developer</h3>
                            <div><a href="https://intimultisolusi.com/" target="_blank">CV. Inti Multi Solusi</a></div>
                            <p>As a full-stack programmer, I have expertise in using various programming languages and frameworks such as Laravel, CodeIgniter, Android Native, and Node.js. My work involves developing various applications and IT solutions for clients, with a focus on government clients such as the Ministry of Communication and Informatics, the Ministry of Investment, and the Ministry of Social Affairs.</p>
                        </div>
                        <div>July 2018 - September 2022</div>
                    </div>
                    <div>
                        <div>
                            <h3>Junior Frontend Developer</h3>
                            <div><a href="https://www.virallo.id/" target="_blank">PT. Seva Space (Virallo)</a></div>
                            <p>As a junior frontend web developer proficient in Angular, my role involves developing web applications to manage investment programs and other applications such as danalaut.id.</p>
                        </div>
                        <div>April 2018 - September 2018</div>
                    </div>
                    <div>
                        <div>
                            <h3>Junior Fullstack Developer Intern</h3>
                            <div><a href="https://artivisi.com/" target="_blank">PT. Artivisi Intermedia</a></div>
                            <p>I am completing an internship as a requirement for graduation from vocational high school, where I am learning to use Java and Laravel programming languages.</p>
                        </div>
                        <div>April 2017 - August 2017</div>
                    </div>
                </div>
            </section>
            <hr />
            <section>
                <div>
                    <h2>Education</h2>
                    <div>
                        <div>
                            <h3>State University of Jakarta</h3>
                            <div>Bachelor of Education</div>
                            <div>Major in Education Technology</div>
                            <p>GPA: 3.69</p>
                        </div>
                        <div>2018 - 2023</div>
                    </div>
                    <div>
                        <div>
                            <h3>SMK Negeri 10 Jakarta</h3>
                            <div>High School Software Engineering</div>
                            <p>GPA: 8.3</p>
                        </div>
                        <div>2016 - 2018</div>
                    </div>
                </div>
            </section>
            <hr />
            <section>
                <div>
                    <h2>Skills</h2>
                    <div>Programming Languages & Tools</div>
                    <ul>
                        <li>Angular</li>
                        <li>Next Js</li>
                        <li>React JS</li>
                        <li>Vue Js</li>
                        <li>Keystone Js</li>
                        <li>Karma/Jasmine</li>
                        <li>Jest</li>
                        <li>Node Js</li>
                        <li>Golang</li>
                        <li>PHP</li>
                        <li>Laravel Framework</li>
                        <li>CodeIgniter Framework</li>
                        <li>Java Spring Boot</li>
                        <li>CSS/SCSS/SASS</li>
                        <li>Flutter</li>
                    </ul>
                    <div>Workflow</div>
                    <ul>
                        <li>Clean Architecture Code</li>
                        <li>Test Driven Development</li>
                        <li>Trunk Development</li>
                        <li>Gitlab CI/CD, Jenskin, SonarQube Scanner</li>
                        <li>Cross Browser Testing & Debugging</li>
                        <li>Cross Functional Teams</li>
                        <li>Agile Development & Scrum Method</li>
                        <li>Systems Development Life Cycle</li>
                        <li>Micro Frontend Architecture</li>
                    </ul>
                </div>
            </section>
            <hr />
            <section>
                <div>
                    <h2>Portfolio</h2>
                    <div>
                        <div>
                            <h3>CROX</h3>
                            <div>PT. Ako Media Asia (SALT)</div>
                            <p>CROX is a website that conducts online testing and usability assessments of digital products and services. Joining as a CROX Tester provides numerous benefits. Register now to enjoy these benefits.</p>
                        </div>
                        <div><a href="https://crox.id/" target="_blank">Link to Project</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>MoGawe Business</h3>
                            <div>PT. Media Baru Internasional (MoGawe)</div>
                            <p>MoGawe Bisnis is a web-based application designed for businesspeople to simplify and manage various types of work in their business. The application is connected to an Android mobile application that offers the opportunity to earn income of at least IDR 200,000 per day by completing simple tasks.</p>
                        </div>
                        <div><a href="https://bisnis.mogawe.id/" target="_blank">Link to Project</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>Adeya Mobile Apps</h3>
                            <div>CV. Inti Multi Solusi (SDPPI Kominfo)</div>
                            <p>To meet the needs of the Company, in the company profile section.</p>
                        </div>
                        <div><a href="https://play.google.com/store/apps/details?id=com.kominfo.sfrsorapp&hl=id" target="_blank">Link to Project</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>E-SIM Drive Thru</h3>
                            <div>CV. Inti Multi Solusi (Polres Satpas Bantul DIY Yogyakarta)</div>
                            <p>This is a web-based technology and application designed to simplify the process of obtaining a driver's license. The system includes a drive-thru feature for creating a SIM, making it faster and more convenient for people to obtain their licenses.</p>
                        </div>
                        <div><a href="#">Internal Use Only</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>Secangkir Ilmu Management</h3>
                            <div>As Individual Project</div>
                            <p>This is a management application designed for internal company use, especially for the HR division. Its features include recording absences, financial records, project progress, and salary information. In general, it functions as an internal company support application.</p>
                        </div>
                        <div><a href="#">Internal Use Only</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>Learning Management System Secangkir Ilmu</h3>
                            <div>As Individual Project</div>
                            <p>Learning Center for Secangkir Ilmu Company, using Moodle.</p>
                        </div>
                        <div><a href="#">Internal Use Only</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>Secangkir Ilmu Company Profile</h3>
                            <div>As Individual Project</div>
                            <p>This service provides comprehensive basic information about companies, presented in an attractive way that is easily accessible to investors and customers.</p>
                        </div>
                        <div><a href="#">Internal Use Only</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>SDS Landing Page (SDS)</h3>
                            <div>CV. Inti Multi Solusi (SDPPI Kementerian Komunikasi dan Informatika)</div>
                            <p>The project of the Ministry of Communication and Information Technology of the Republic of Indonesia in making a landing page application to display information from table. using YII & Tableu</p>
                        </div>
                        <div><a href="#">Internal Use Only</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>DATA VALIDATION OF FREQUENCY RADIO (ISR QR CODE)</h3>
                            <div>CV. Inti Multi Solusi (SDPPI Kementerian Komunikasi dan Informatika)</div>
                            <p>A project of the Indonesian Ministry of Communication and Information Technology in making radio station registration applications and permits as well as validating radio station licenses, using Laravel.</p>
                        </div>
                        <div><a href="https://isrcode.postel.go.id/" target="_blank">Link to Project</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>Website Certification (REOR)</h3>
                            <div>CV. Inti Multi Solusi (SDPPI Kementerian Komunikasi dan Informatika)</div>
                            <p>The project of the Ministry of Communication and Information Technology of the Republic of Indonesia in making certification and training print, using PHP YII.</p>
                        </div>
                        <div><a href="https://reor.postel.go.id/" target="_blank">Link to Project</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>COMPUTER ASSISTED TEST (CAT)</h3>
                            <div>CV. Inti Multi Solusi (SDPPI Kementerian Komunikasi dan Informatika)</div>
                            <p>A project of the Indonesian Ministry of Communication and Information Technology in making Computer Assisted Test, using Laravel.</p>
                        </div>
                        <div><a href="https://cat.postel.go.id/" target="_blank">Link to Project</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>REPORTING WEBSITE PROJECT (UPT)</h3>
                            <div>CV. Inti Multi Solusi (SDPPI Kementerian Komunikasi dan Informatika)</div>
                            <p>A project of the Indonesian Ministry of Communication and Information Technology in making radio station reporting and license applications, using Laravel.</p>
                        </div>
                        <div><a href="https://layananupt.ditfrek.postel.go.id/login" target="_blank">Link to Project</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>PROJECT WEBSITE SERVICES (Postel)</h3>
                            <div>CV. Inti Multi Solusi (SDPPI Kementerian Komunikasi dan Informatika)</div>
                            <p>The project of the Indonesian Ministry of Communication and Information Technology in updating and developing services in the postal section, using Laravel.</p>
                        </div>
                        <div><a href="#">Not Active</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>QUEUE SYSTEM</h3>
                            <div>CV. Inti Multi Solusi (BKPM or Kementrian Investasi)</div>
                            <p>Investment Coordinating Board project in creating an online consultation queue system, using Laravel.</p>
                        </div>
                        <div><a href="https://antrian.bkpm.go.id/" target="_blank">Link to Project</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>INTERNAL COOPERATIVE APPLICATION</h3>
                            <div>CV. Inti Multi Solusi (Kementerian Sosial)</div>
                            <p>The project of the Ministry of Social Affairs of the Republic of Indonesia in making internal cooperative applications to improve performance, using PHP Codeigniter</p>
                        </div>
                        <div><a href="#">Internal Use Only</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>COMPANY PROFILE CBI DKI</h3>
                            <div>As Individual Project</div>
                            <p>To meet the needs of the Company, in the company profile section.</p>
                        </div>
                        <div><a href="#">Not Active</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>INVESTMENT PROJECT DANALAUT</h3>
                            <div>PT. SEVA SPACE</div>
                            <p>To meet the needs of companies engaged in investing in a business from these regions, using Angular.</p>
                        </div>
                        <div><a href="#">Not Active</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>VIRALLO</h3>
                            <div>PT. SEVA SPACE</div>
                            <p>To meet the needs of companies engaged in advertisers and influencers, using Laravel & Angular.</p>
                        </div>
                        <div><a href="https://www.virallo.id/" target="_blank">Link to Project</a></div>
                    </div>
                    <div>
                        <div>
                            <h3>DUNIA HALAL</h3>
                            <div>PT. ARTIVISI INTERMEDIA</div>
                            <p>E-commerce applications engaged in Online Stores (such as tokopedia.com etc.), using Java.</p>
                        </div>
                        <div><a href="#">Not Active</a></div>
                    </div>
                </div>
            </section>
            <hr />
            <section>
                <div>
                    <h2>Awards & Certifications</h2>
                    <ul>
                        <li>Sci-Fi Hardware Hackathon</li>
                        <li>Sertifikasi Garuda Emas BNSP</li>
                        <li>Politeknik LP3I Jakarta Campus</li>
                        <li>Establish an educational organization to provide courses to the general public in Indonesia (Secangkir Ilmu)</li>
                        <li>Resource persons at the ministry of communication and information related to the Reporting Application</li>
                        <li>Resource persons at the ministry of communication and information related to the GIS Application</li>
                        <li>Resource persons at the ministry of communication and information related to the Computer Assisted Test</li>
                        <li>As a speaker "How To Start Professional Career as Web Developer" At Klasspro.id</li>
                        <li>As a speaker "Menghadapi Era Society 5.0 dengan Skill Digital" At Secangkir Ilmu</li>
                        <li>HackerRank Skills verification test for Angular</li>
                        <li>Tutors Backend Developer Using PHP At Secangkir Ilmu</li>
                        <li>Tutors AR/VR Using Unity At Secangkir Ilmu</li>
                        <li>LinkedIn Skill Badge: Angular</li>
                        <li>LinkedIn Skill Badge: PHP</li>
                        <li>LinkedIn Skill Badge: HTML</li>
                        <li>LinkedIn Skill Badge: MySQL</li>
                        <li>LinkedIn Skill Badge: C++</li>
                        <li>LinkedIn Skill Badge: CSS</li>
                        <li>LinkedIn Skill Badge: Front-End Development</li>
                        <li>LinkedIn Skill Badge: Git</li>
                        <li>LinkedIn Skill Badge: Go</li>
                        <li>LinkedIn Skill Badge: Java</li>
                        <li>LinkedIn Skill Badge: Javascript</li>
                        <li>LinkedIn Skill Badge: jQuery</li>
                        <li>LinkedIn Skill Badge: JSON</li>
                        <li>LinkedIn Skill Badge: MySQL</li>
                        <li>LinkedIn Skill Badge: React.js</li>
                        <li>LinkedIn Skill Badge: REST APIs</li>
                    </ul>
                </div>
            </section>
        </div>
    </body>
</html>
